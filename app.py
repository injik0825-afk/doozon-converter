...
def classify_enra_hana(month, day, in_amt, out_amt, memo, partner, combined, account_id):
    ...
    if account_id == '02504':
        ...
        if any(k in combined for k in ['운용보수','성과보수','용역대가','수수료']):
    ...
    if account_id == '67204':
        ...
        if '경영관리' in memo:
            return []
        # 청약 출금 (하이일드인벤테라 등 - 67204에서만 보이는 펀드 청약 흐름)
    ...
    if out_amt > 0:
        ...
        # 코스콤중계서비스, 크레탑이용료, 증권데이터가이드, 펀드보고서비용 등 = 전산사용료
        if any(k in memo for k in ['코스콤','크레탑','데이터가이드','펀드보고서','시세']):
            rows += [row(month, day, '차변', '전산사용료', '', memo, out_amt, 0),
                     row(month, day, '대변', '보통예금',   cp_self, memo, 0, out_amt)]
            return rows

        # 회비 (정회원가입회비, 협회비)
        if '회비' in memo or '협회' in combined:
            rows += [row(month, day, '차변', '세금과공과금', '', memo, out_amt, 0),
                     row(month, day, '대변', '보통예금',     cp_self, memo, 0, out_amt)]
            return rows
...
def process_enra_corp_card(df):
    ...
    for _, r in df.iterrows():
        ...
        if any(k in biz for k in ['일반음식점','일식','중식','서양식','패스트푸드',
            ...
            acct = '복리후생비'
        elif '백화점' in biz or '슈퍼마켓' in biz or '대형할인점' in biz:
            acct = '복리후생비'
        elif '호텔' in biz or '여관' in biz or '숙박' in biz:
            acct = '여비교통비'
        elif '골프' in biz:
            acct = '접대비'  # 골프장은 접대비
        elif 'PG일반' in biz or '온라인쇼핑' in biz:
            acct = '지급수수료'
